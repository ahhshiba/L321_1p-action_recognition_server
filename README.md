...
1. `raw_restreamer` 以 FFmpeg 轉推 USB 攝影機 (`/dev/video0`) 到 go2rtc。
2. `app/action_recognition_server.py` 拉取 `cam1_raw` 串流、在畫面上疊加行為辨識結果，並重新輸出成 `cam1_overlay`。
3. [`go2rtc`](https://github.com/AlexxIT/go2rtc) 將 RTSP 串流轉成 WebRTC/HTTP 以供使用者端播放。
4. 兩個前端 (`html_test` 與 `html16`) 讀取 `share/*.json` 描述的相機/事件/錄影資訊並呈現成儀表板。
...
- **新增相機**：
  1. 在 `go2rtc/go2rtc-config.yaml` 新增 stream 定義。
  2. 更新 `docker-compose.yml`（必要時）與 `share/cameras.json`。
  3. 重啟 `go2rtc` 與 `action_recognition_server`。
